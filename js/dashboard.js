const Dashboard = {
    ...
    renderUserBars(txs) {
        ...
        if (!el) return;
        const nTotal = txs.filter(t => t.type === 'expense' && t.paid !== false && t.userId === 'nadia').reduce((s, t) => s + t.amount, 0);
        const eTotal = txs.filter(t => t.type === 'expense' && t.paid !== false && t.userId === 'elias').reduce((s, t) => s + t.amount, 0);
